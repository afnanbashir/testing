...
class Inferencer(object):
    ...
    def load_model(self):
        print('Load model from {self.args.model}')
        #checkpoint = torch.load(f'{self.args.model}', map_location=lambda storage, location: 'cpu')
        checkpoint =  torch.load(f'{self.args.model}', map_location=torch.device('cpu'))
    ...
    def inference_from_path(self):
        src_mel, _ = get_spectrograms(self.args.source)# mel frequency cepstrum is created here
...
if __name__ == '__main__':
    ...
    parser.add_argument('-attr', '-a', default='attr.pkl')
    parser.add_argument('-config', '-c', default='config.yaml')
    parser.add_argument('-model', '-m', default='vctk_model.ckpt')
